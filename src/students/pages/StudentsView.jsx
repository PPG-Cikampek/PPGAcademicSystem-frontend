...
const StudentsView = () => {
    ...
    useEffect(() => {
        const url = auth.userRole === 'admin'
            ...
            : auth.userRole === 'branchAdmin'
                ? `${import.meta.env.VITE_BACKEND_URL}/students/branch/${auth.userBranchId}`
                : `${import.meta.env.VITE_BACKEND_URL}/students/sub-branch/${auth.userSubBranchId}`;
    ...
    const columns = [
        ...
        ...(auth.userRole !== 'admin' ? [{
            ...
            cellStyle: (student) => `py-1 px-2 text-sm text-center w-min border rounded-md ${!student.isActive ? 'text-red-500 bg-red-100' : 'text-green-500 bg-green-100'
                }`
        }] : []),
        { key: 'nis', label: 'NIS', sortable: true },
        { key: 'name', label: 'Nama', sortable: true },
        {
            key: 'isInternal', label: 'Label', sortable: true,
            render: (student) => student.isInternal ? 'Internal' : 'Simpatisan',
            cellStyle: (student) => `py-1 px-2 text-sm text-center w-min border rounded-md ${student.isInternal ? 'text-blue-500 bg-blue-100' : 'text-gray-500 bg-gray-100'
                }`
        },
        ...(auth.userRole === 'admin' ? [
            {
    ...
    if (students?.length > 0) {
        const branches = [...new Set(students.map(s => s?.userId?.subBranchId?.branchId?.name).filter(Boolean))];
        const subBranches = [...new Set(students.map(s => s?.userId?.subBranchId?.name).filter(Boolean))];
