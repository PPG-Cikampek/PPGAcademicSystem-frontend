        ...
        <Page key={idx} size="A4" style={styles.page}>
          <View style={styles.header}>
            ...
            <View style={styles.institution}>
              <Text style={{ fontSize: 14, fontWeight: 'bold' }}>PENGGERAK PEMBINA GENERUS</Text>
              <Text style={{ fontSize: 18, fontWeight: 'bold' }}>(PPG)</Text>
              <Text style={{ fontSize: 14 }}>DAERAH CIKAMPEK</Text>
          ...
          <View>
            <Text style={styles.title}>
              SOAL MUNAQOSAH SEMESTER {state.semester.toString().endsWith('1') ? 'GANJIL' : 'GENAP'}
            </Text>
            <Text style={styles.subtitle}>
              TAHUN {state.semester.toString().slice(0, 4)}/{parseInt(state.semester.toString().slice(0, 4)) + 1}
            </Text>
            <Text style={styles.info}>Materi : {getCategoryName(state.category)}</Text>
            <Text style={styles.info}>Kelas : {kelasObj.classGrade}</Text>
          </View>
          <View style={styles.table}>
            <View style={styles.tableRow}>
              <Text style={[styles.tableColHeader, styles.colNo]}>No</Text>
              <Text style={[styles.tableColHeader, styles.colQuestion]}>Pertanyaan</Text>
              <Text style={[styles.tableColHeader, styles.colScore]}>Skor</Text>
            </View>
            {(kelasObj.questions || []).map((q, i) => (
              <View style={styles.tableRow} key={i}>
                <Text style={[styles.tableCol, styles.colNo]}>{i + 1}</Text>
                <Text style={[styles.tableCol, styles.colQuestion]}>
                  {q.question}
                  {q.answers && q.answers.length ? "\n\n" + q.answers.join('\n') : ''}
                </Text>
                <Text style={[styles.tableCol, styles.colScore]}>
