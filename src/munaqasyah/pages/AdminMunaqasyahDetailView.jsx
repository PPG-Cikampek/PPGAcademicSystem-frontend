...
const AdminMunaqasyahDetailView = () => {
    const { sendRequest, isLoading: isMutating, error: httpError, setError } =
        useHttp();
    const { modalState, openModal, closeModal } = useModal();
    const auth = useContext(AuthContext);
    const navigate = useNavigate();
    const { branchId: paramBranchId, branchYearId } = useParams();
    const branchId = paramBranchId;
    const location = useLocation();
    const state = location.state?.year ? location.state : {};
    ...
    const handleMonitor = (subBranchId, subBranchMunaqasyahStatus) => {
        navigate(
            `/munaqasyah/monitor/${branchYearId}/branch/${branchId}/sub-branch/${subBranchId}`,
            {
                state: {
                    branchYearId,
                    subBranchId,
                    ...
                    <>
                        <div className="mb-8">
                            <div className="flex flex-col gap-2">
                                <h1 className="font-semibold text-gray-900 text-2xl">
                                    {state?.branchName || state?.year?.branchId?.name || "Cabang"}
                                </h1>
                                <p className="text-gray-600">
                                    Tahun Ajaran: {state?.year?.academicYearId?.name || "-"}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="mb-8">
                            ...
                            <DataTable
                                ...
                                tableId={`admin-branch-sub-branches-table-${branchYearId}`}
                            />
                        </div>
                    </>
                )}
...
export default AdminMunaqasyahDetailView;
