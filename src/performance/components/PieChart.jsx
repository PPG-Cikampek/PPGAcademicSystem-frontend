...
const PieChart = memo(
    ({ attendanceData, chartType, toImage = false }) => {
    ...
    (prevProps, nextProps) => {
        ...
        if (!prevProps.attendanceData || !nextProps.attendanceData)
            return false;

        // Deep comparison of attendanceData array
        if (prevProps.attendanceData.length !== nextProps.attendanceData.length)
            return false;

        return (
            prevProps.attendanceData.every((item, index) => {
