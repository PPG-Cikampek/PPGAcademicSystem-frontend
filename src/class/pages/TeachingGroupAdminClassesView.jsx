...
const TeachingGroupAdminClassesView = () => {
    ...
    const columns = [
        ...
        { key: 'teachers', label: 'Guru', sortable: true, render: (row) => row.teachers?.length ?? row.teachers },
        { key: 'students', label: 'Siswa', sortable: true, render: (row) => row.students?.length ?? row.students },
        { key: 'isLocked', label: 'Status', sortable: true, render: (row) => row.isLocked ? 'Terkunci' : 'Terbuka' },
