...
const AuthView = () => {
    ...
    const handleFormSubmit = async (data) => {
        ...
        try {
            ...
            try {
                branchYearData = await sendRequest(
                    `${import.meta.env.VITE_BACKEND_URL}/branchYears/branch/${
                        responseData.user.subBranchId.branchId.id
                    }`
                );
            ...
            if (branchYearData && branchYearData.branchYears) {
                activeBranchYear = branchYearData.branchYears.find(
                    (year) => year.academicYearId.isActive
                );
            }

            let teacherData;
            if (
                responseData.user.role === "teacher" ||
                responseData.user.role === "munaqisy"
            ) {
                try {
                    teacherData = await sendRequest(
            ...
            const classIds =
                teacherData?.teacher?.classIds.map((item) => item._id) || [];
                ...
                {isStudent === true && (
                    <DynamicForm
                        ...
                        helpButton={
                            studentLoginFields.length === 0 && (
                                <QRScanner
                                    setStudentLoginField={setStudentLoginFields}
                                    setNis={setNis}
                                />
                            )
                        }
