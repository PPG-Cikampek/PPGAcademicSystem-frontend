...
const RequestAccountForm = () => {
    ...
    const teacherFields = [
        { name: 'name', label: 'Nama', placeholder: 'Nama Lengkap', type: 'text', required: true },
        { name: 'dateOfBirth', label: 'Tanggal Lahir', placeholder: 'Tanggal Lahir', type: 'date', required: true },
    ...
    const studentFields = [
        ...
        { name: 'dateOfBirth', label: 'Tanggal Lahir', placeholder: 'Tanggal Lahir', type: 'date', required: true },
        {
            name: 'className', label: 'Kelas', placeholder: 'Pilih Kelas', type: 'select', required: true, options: [
            ...
            <div className="card-basic rounded-md items-stretch flex-col my-12 mx-48">
                <div className="pb-24 transition-opacity duration-300">
                    ...
                    <form className="flex flex-col md:flex-row gap-4 items-end mb-12">
                        {fields.map((field) => (
                            <div key={field.name} className="flex flex-col w-full">
                                <label htmlFor={field.name} className="block text-gray-700 pb-1">{field.label}</label>
                                {field.type === 'select' ? (
                                    <select
                                        id={field.name}
                                        name={field.name}
                    ...
                    {dataList.length > 0 && (
                        <>
                            <div>
                                <h3 className='text-lg mt-1 font-normal'>List akun yang akan dibuat:</h3>
                                <table className="min-w-full mt-4 border-collapse border border-gray-200">
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="border border-gray-200 p-2 text-left font-normal text-gray-500">{isStudent ? 'KELAS' : 'EMAIL'}</th>
                                    ...
                                    <tbody>
                                        {dataList.map((data, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className="border border-gray-200 p-2">{isStudent ? data.className : data.email}</td>
                            ...
                            <button
                                ...
                                disabled={isLoading}
                            >
                                {isLoading ? (<LoadingCircle>Processing...</LoadingCircle>) : 'Buat Permintaan Akun'}
                            </button>
                        </>
                    )}
                </div>
